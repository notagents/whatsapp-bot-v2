...
type Props = {
  ...
  responsesEnabled: MonitorConversationDetail["responsesEnabled"];
};

export function ContextTab({ memory, responsesEnabled }: Props) {
  return (
      ...
      <Card>
        ...
        <CardContent className="space-y-1 py-0">
          <p>
            Respuestas:{" "}
            <Badge
              variant={responsesEnabled?.enabled ? "success" : "destructive"}
            >
              {responsesEnabled?.enabled ? "Sí" : "No"}
            </Badge>
          </p>
          {responsesEnabled?.disabledUntilUTC && (
            <p className="text-muted-foreground">
              Deshabilitado hasta: {responsesEnabled.disabledUntilUTC}
            </p>
          )}
        </CardContent>
      </Card>
