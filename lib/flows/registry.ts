...
async function loadFlowFromPath(dir: string): Promise<{ config: FlowConfig; flowPath: string } | null> {
  const flowPath = getFlowPath(dir);
  try {
    const raw = await readFile(flowPath, "utf-8");
    const parsed = JSON.parse(raw) as unknown;
    const config = flowConfigSchema.parse(parsed);
    const flowPathRelative = flowPath.replace(process.cwd(), "").replace(/\\/g, "/");
    return { config, flowPath: flowPathRelative || `/${FLOWS_BASE}/default/${FLOW_FILE}` };
  } catch {
    return null;
  }
}
